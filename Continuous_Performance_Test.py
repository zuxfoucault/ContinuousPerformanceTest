...
import CPT_data
from dialog import Ui_ExperimentInfo
...
expName = u'Continuous Performance Task'  # from the Builder filename that created this script
expInfo = {u'Sex': u'female',
           ...
           u'Activity Level': u'Medium',
           u'# of runs per experiment': u'2'}
...
ExperimentInfo.show()
app.exec_()
expInfo = {u'Sex': ui.sex.text(),
           u'Handedness': ui.hand.text(),
           u'Participant': ui.pid.text(),
           u'Session': ui.sess.text(),
           u'Age': ui.age.text(),
           u'Years of completed education': ui.edu.text(),
           u'Activity Level': ui.actl.text(),
           u'# of runs per experiment': ui.runs.text()}

for key, val in expInfo.iteritems():
    expInfo[key] = str(val)

#sys.exit(app.exec_())
#expInfo = dlg.exp_info
...
if expInfo[u'Session'] == u'1':
    session1_date = expInfo[u'date'][9:11] + '-' + expInfo[u'date'][5:8] + '-' + expInfo[u'date'][0:4]
    session2_date = "."
    session1 = "X"
    session2 = "."
else:
    session1_date = "."
    session2_date = expInfo[u'date'][9:11] + '-' + expInfo[u'date'][5:8] + '-' + expInfo[u'date'][0:4]
    session1 = "."
    session2 = "X"
